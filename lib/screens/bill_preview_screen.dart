...
class _BillPreviewScreenState extends State<BillPreviewScreen> {
  ...
  Future<List<int>> _buildPdfBytes(
    ...
    final cellBold      = pw.TextStyle(font: ttfBold,    fontSize: 7);
              ...
              pw.Table(
                ...
                children: [
                  ...
                  ...bill.entries.asMap().entries.map((ep) {
                    ...
                    // Use Rs. in PDF amount column (avoids font-glyph issues)
                    ...
                    children: [
                      ...
                      pw.Row(
                        ...
                        children: [
                          ...
                          pw.Text(_pdfAmt(fmtINR2(t.net)), style: netStyle),
                        ],
                      ),
                    ],
                  ),
  ...
  Future<void> _downloadExcel() async {
    ...
    try {
      ...
      void addRow(List<String> values, {bool bold = false, String? bg}) {
      ...
      for (int i = 0; i < bill.entries.length; i++) {
        ...
        addRow([
          ...
          e.destination,
          e.clientName,
          e.price.isEmpty ? '0' : e.price, // plain number — no symbol needed
        ]);
      }

      addRow(['']);

      // ── Totals: _ea() replaces ₹ with Rs. ──
      addRow(['', '', '', '', '', '', 'Gross Total:', _ea(fmtINR2(totals.gross))],
          bold: true);
      if (totals.fuelPct > 0) {
        addRow([
          '', '', '', '', '', '',
          '${(totals.fuelPct * 100).toStringAsFixed(0)}% Fuel Charges:',
          _ea(fmtINR2(totals.fuel))
        ]);
      }
      addRow(['', '', '', '', '', '', 'Total Value of Supply:',
          _ea(fmtINR2(totals.ts))]);
      if (totals.cgstPct > 0) {
        addRow([
          '', '', '', '', '', '',
          '${(totals.cgstPct * 100).toStringAsFixed(0)}% CGST:',
          _ea(fmtINR2(totals.cgst))
        ]);
      }
      if (totals.sgstPct > 0) {
        addRow([
          ...
          _ea(fmtINR2(totals.sgst))
        ]);
      }
      addRow(
          ['', '', '', '', '', '', 'NET AMOUNT:', _ea(fmtINR2(totals.net))],
