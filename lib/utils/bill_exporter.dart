...
<tr>
  ...
  <td>${e.destination}</td>
  <td>${e.clientName}</td>
  <td class="r">₹${e.price.isEmpty ? '0' : e.price}</td>
</tr>''';
  }
  ...
  if (t.fuelPct > 0) {
    taxRows +=
        '<div class="trow"><span class="tl">${(t.fuelPct * 100).toStringAsFixed(0)}% Fuel Charges</span><span class="tv">${fmtINR2(t.fuel)}</span></div>';
  }
  taxRows +=
      '<div class="trow"><span class="tl">Total Value of Supply</span><span class="tv">${fmtINR2(t.ts)}</span></div>';
  if (t.cgstPct > 0) {
    taxRows +=
        '<div class="trow"><span class="tl">${(t.cgstPct * 100).toStringAsFixed(0)}% CGST</span><span class="tv">${fmtINR2(t.cgst)}</span></div>';
  }
  if (t.sgstPct > 0) {
    taxRows +=
        '<div class="trow"><span class="tl">${(t.sgstPct * 100).toStringAsFixed(0)}% SGST</span><span class="tv">${fmtINR2(t.sgst)}</span></div>';
  }
...
<div class="bar2"></div>
<div class="totals">
  <div class="trow"><span class="tl">Gross Total</span><span class="tv">${fmtINR2(t.gross)}</span></div>
  $taxRows
  <div class="trow tnet"><span class="tl">NET AMOUNT</span><span class="tv">${fmtINR2(t.net)}</span></div>
</div>
<div class="bar2"></div>
<div class="foot">
  <div class="bank">
